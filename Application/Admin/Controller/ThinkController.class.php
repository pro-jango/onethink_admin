...
class ThinkController extends AdminController {
    ...
    public function del($modelname = null, $ids=null){
        $model = M('Model')->find($modelname);
        $model || $model =  M('Model')->getByName($modelname);
        $model || $this->error('模型不存在！');
    ...
    public function edit($modelname = null, $id = 0){
        //获取模型信息
        $model = M('Model')->find($modelname);
        $model || $model =  M('Model')->getByName($modelname);
        $model || $this->error('模型不存在！');
    ...
    public function add($modelname = null){
        ...
        //获取模型信息
        $model = M('Model')->where(array('status' => 1))->find($modelname);
        $model || $model =  M('Model')->getByName($modelname);
        $model || $this->error('模型不存在！');
        if(IS_POST){
            $Model  =   D(parse_name(get_table_name($model['id']),1));
    ...
    public function buildPage($count){
        //分页
        $pageRow = !empty($this->pageRow) ? $this->pageRow : 20;
        if($count > $this->pageRow){
            $page = new \Think\Page($count, $this->pageRow);
            $page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %HEADER%');
            $this->assign('_page', $page->show());
        }
    }

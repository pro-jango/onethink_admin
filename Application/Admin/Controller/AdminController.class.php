...
class AdminController extends Controller {
    ...
    final protected function returnNodes($tree = true){
        ...
        $map['is_dev'] =   0;
        if((int)$tree){
            $list = M('Menu')->where($map)->field('id,pid,title,url,tip,hide')->order('sort asc')->select();
            ...
            $nodes = M('Menu')->where($map)->field('title,url,tip,pid')->order('sort asc')->select();
            foreach ($nodes as $key => $value) {
                if( stripos($value['url'],MODULE_NAME)!==0 ){
    ...
    protected function ajaxExit($result,$msg,$data=array()){
        $callback = isset($_GET['callback']) ? htmlspecialchars($_GET['callback']) : '';
        $out['result'] = $result;
        $out['msg'] = $msg;
        $out['data'] = $data;
        $outinfo = json_encode($out);
        if($callback){
            die("$callback({$outinfo})");
        }else{
            die($outinfo);
        }
    }
